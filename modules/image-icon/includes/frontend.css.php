		...
		.fl-node-<?php echo esc_attr( $id ); ?> .uabb-icon-wrap .uabb-icon i:before {
			...
			<?php if ( 'simple' !== $settings->icon_style ) { // Rounded Styles. ?>
				...
				line-height:
					<?php
					echo (
					( ( 'custom' !== $settings->icon_style ) ? esc_attr( ( $settings->icon_size * 2 ) ) : esc_attr( $settings->icon_size ) )
					+
					( ( 'custom' === $settings->icon_style ) ? esc_attr( $settings->icon_bg_size ) : 0 )
							. 'px;' );
					?>
...
<?php if ( $global_settings->responsive_enabled ) { ?>
	<?php if ( $version_bb_check ) { ?>
		@media ( max-width: <?php echo esc_attr( $global_settings->medium_breakpoint ) . 'px'; ?> ) {
			.fl-node-<?php echo esc_attr( $id ); ?> .uabb-imgicon-wrap {
				<?php if ( 'icon' === $settings->image_type ) { ?>
					text-align: <?php echo esc_attr( $settings->icon_align_medium ); ?>;
				<?php } elseif ( 'photo' === $settings->image_type ) { ?>
					text-align: <?php echo esc_attr( $settings->img_align_medium ); ?>;
				<?php } ?>
			}
		}
		@media ( max-width: <?php echo esc_attr( $global_settings->responsive_breakpoint ) . 'px'; ?> ) {
			.fl-node-<?php echo esc_attr( $id ); ?> .uabb-imgicon-wrap {
				<?php if ( 'icon' === $settings->image_type ) { ?>
					text-align: <?php echo esc_attr( $settings->icon_align_responsive ); ?>;
				<?php } elseif ( 'photo' === $settings->image_type ) { ?>
					text-align: <?php echo esc_attr( $settings->img_align_responsive ); ?>;
				<?php } ?>
			}
		}
	<?php } ?>
